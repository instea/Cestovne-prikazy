...
import { FilterLeaves, ClearLeavesFilter, LeaveListFilter } from '../../state/leaves';
...
function makeRequesterOptions(leaves) {
  const users = uniqBy(
    leaves.map(leave => leave.requester),
    user => user.id
  );
...
export class LeavesListComponent implements OnInit {
  ...
  ngOnInit() {
    ...
    this.filteredLeaves$ = Observable.combineLatest(
      leaves$, filter$, this.filterLeaves
    );
  ...
  updateFilterState = model => {
    this.store.dispatch(new FilterLeaves({
